...
public:
	...
	*/
	virtual void See(DWORD dwUnk, DWORD dwUnk2) { JMP_THIS(0x5F4340); }
	...
	*/
	virtual bool IsCloseEnough3D(DWORD dwUnk, DWORD dwUnk2) const JMP_THIS(0x5F4380);
	...
	*/
	virtual DamageState ReceiveDamage(int* pDamage, int DistanceFromEpicenter, WarheadTypeClass* pWH, ObjectClass* Attacker, bool IgnoreDefenses, bool PreventPassengerEscape, HouseClass* pAttackingHouse) JMP_THIS(0x5F5390);
	...
	*/
	...
	double GetHealthPercentage() const
		{ return static_cast<double>(this->Strength) / this->Type()->MaxStrength; }
...
public:
	...
	*/
	inline DirStruct Direction(AbstractClass* pTarget) const JMP_THIS(0x5F3DB0);
	/*!
	* @note in EXE this is method of ObjectClass, but it can be used with any Abstract.
	*/
	inline int DistanceFrom(AbstractClass* that) const JMP_THIS(0x5F6440);
	/*!
	* @note in EXE this is method of ObjectClass, but it can be used with any Abstract.
	*/
	inline int DistanceFrom3D(AbstractClass* that) const JMP_THIS(0x5F6360);
};
